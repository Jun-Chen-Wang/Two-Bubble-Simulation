...
# -*- coding: utf-8 -*-
"""
Created on Fri May  9 09:08:09 2025
...
class Two_Bubble_Thick_Calculation(object):
    delta_r = 10**(-3)
    nT_sample_plot = 400
    nZ_sample_plot = 400
    nR_sample_plot = 5
    ...
    def __init__(self,w,initial_profile,d,T,Z,R,NT,NZ,NR,device,dtype):
        self.device = device
        self.dtype = dtype
    ...
    def initial_phi(self):
        x = self.initial_profile[:,0]
        y = self.initial_profile[:,1]
        
        dis_1 = torch.sqrt(self.r_tensor**2+(self.z_tensor-self.d/2)**2)
        dis_2 = torch.sqrt(self.r_tensor**2+(self.z_tensor+self.d/2)**2)
        
        value_1 = np.interp(dis_1.cpu(), x.cpu(), y.cpu())
        value_2 = np.interp(dis_2.cpu(), x.cpu(), y.cpu())
